...
<template>
  <div class="flex justify-center items-start min-h-[80vh] py-10 bg-slate-50">
    <div
      class="w-full max-w-md bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden"
    >
      <div class="p-6 border-b border-slate-100 space-y-1.5">
        ...
        <p class="text-sm text-slate-500">Top up your account to play multiplayer matches.</p>
      </div>

      <div class="p-6 space-y-6">
        <div
          class="bg-slate-50 p-4 rounded-lg flex items-center justify-between border border-slate-200"
        >
        ...
        <form @submit.prevent="buyCoins" class="space-y-4">
          ...
          <div class="space-y-2">
            ...
            <p class="text-xs text-slate-500">Example: {{ referencePlaceholder }}</p>
          </div>

          <Button
            type="submit"
            class="w-full h-12 text-lg font-semibold bg-green-600 hover:bg-green-700 text-white"
            :disabled="loading"
          >
      ...
      <div v-if="message || error" class="p-6 pt-0 flex flex-col gap-2">
        <div
          v-if="message"
          class="p-4 rounded-md bg-green-50 text-green-800 border border-green-200 text-sm font-medium"
        >
          ✅ {{ message }}
        </div>
        ...
        <div
          v-if="error"
          class="p-4 rounded-md bg-red-50 text-red-800 border border-red-200 text-sm font-medium"
        >
          ❌ {{ error }}
        </div>
      </div>
